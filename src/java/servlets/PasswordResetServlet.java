...
public class PasswordResetServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("input_reset") == null) {
            // Simply redirect the user to the password reset page
        ...
        if (username.isEmpty() || password.isEmpty()) {
            request.setAttribute("message", "A username and password is required.");
            getServletContext().getRequestDispatcher("/password_reset.jsp").forward(request, response);
            return;
        }
        ...
        if (session.getAttribute("user") == null) {
            request.setAttribute("message", "The user '" + username + "' does not exist.");
            getServletContext().getRequestDispatcher("/password_reset.jsp").forward(request, response);
            return;
        }
